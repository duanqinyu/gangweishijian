...
export default function Dashboard() {
  ...
  const canAccessStudents = user?.role === 'admin' || user?.role === 'teacher';

  const cards = [
    ...(canAccessStudents ? [{
      icon: Users,
      title: '学生',
      count: `${students.length} 名`,
      subtitle: '学生',
      path: '/students'
    }] : []),
    {
  ...
  const recentActivities = getRecentActivities(3).slice(0, 4);
